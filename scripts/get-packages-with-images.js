...
/**
 * Filter packages to the ones that have a "deploy-image" package.json command.
 * @param {{
 *  name: string;
 *  version: string;
 *  path: string;
...
async function filterContainerizable(packages) {
  const filtered = (await Promise.all(packages.map(async (package) => {
    const packageJson = JSON.parse(await fs.readFile(`${package.path}/package.json`));
    if (packageJson.scripts['deploy-image']) {
      return {
        name: package.name,
...
async function main() {
  ...
  const packagesWithPaths = await getPackagePaths(packages.map(p => p.name));
